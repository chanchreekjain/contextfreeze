...
**Costs:** Microsoft Edge Add-ons charges no registration fee. The Chrome Web
Store charges a one-time developer registration fee. Edge is Chromium, so the
same zip uploads to both — publishing to Edge first costs nothing and puts the
listing in front of a reviewer before any money changes hands.

**Edge certification takes up to 7 business days.** Chrome states most reviews
finish in days but can take weeks, and lists broad host permissions, `tabs` and
`downloads` as things that lengthen a review — this extension has all three, on
a new account, which is every slow-review flag at once. Nothing is wrong with
that; it just means the permission justifications below carry more weight than
usual.
...
| File | Size | Chrome | Edge |
|---|---|---|---|
| `icon-store-128.png` | 128×128, padded | required | — |
| `logo-store-300.png` | 300×300, full bleed | — | required |
| `promo-small-440x280.png` | 440×280 | required | optional |
| `promo-marquee-1400x560.png` | 1400×560 | optional | optional |

The two icons are not interchangeable. Chrome wants the artwork inset with
transparent padding; Edge renders a 1:1 logo as-is. Shipping one where the other
belongs looks subtly wrong and nobody can tell you why.

### Screenshots

Yours to take — Chrome accepts 1280×800 or 640×400, Edge 1280×800 or 640×480, so
**1280×800 satisfies both**. Chrome allows up to 5, Edge up to 6.
